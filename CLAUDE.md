...
- **`{.prototype: "<C prototype>".}`** (RFC-0001 §3 A.1, slice A1 landed): a vendored C declaration copied from upstream's header — string literal (triple-quoted/multi-line accepted), single non-variadic prototype, name must match the proc's C name. A shared tokenizer (`tokenizePrototype`/`analyzePrototype` in `src/softlink.nim`) extracts the identifier immediately before the first depth-0 `(`; a `(*` there means a function-pointer return type (name nested, unextractable) and is rejected with typedef guidance. Lifts the `header` requirement (like `noverify`, though the two are mutually exclusive — both select a declaration source); may coexist with `header` for cross-checking. As of A1 the pragma only parses/validates — emitting the `extern` declaration into the verify TU is slice A2, so a `prototype`-only (no `header`) proc is currently just unverified (like `noverify`, without needing the pragma) until A2 lands.
- **Pragma allowlist**: only calling conventions + `optional` + `noverify` + `verifyWhen` + `header` + `prototype` are accepted. Unsupported pragmas (e.g., `varargs`) produce compile-time errors.
