...
4. **Generates** a `loadXxx(): LoadResult` proc that:
   - Calls `std/dynlib.loadLib(pattern)` with version fallback
   - For each declared proc, calls `symAddr` and casts to the typed function pointer
   - Returns `lrOk` on success, `lrLibNotFound` if the library is missing, or `lrSymbolNotFound` (with the symbol name) if a symbol can't be resolved
...
The compiler cannot verify against C headers (that requires `{.importc, header.}`). But the macro eliminates the error-prone manual `cast` step that `std/dynlib` requires. Your test suite verifies ABI compatibility.
